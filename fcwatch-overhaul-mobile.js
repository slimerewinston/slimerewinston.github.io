...
// Built: 2026-03-09T17:55:30.113Z
// =====================================================
(function() {
    'use strict';
...
(function () {
    ...
    `;
        ...
        (function inlineSmartMenusOverride() {
            (function () {
    // Wait for the ACTUAL SmartMenus instances on ul.sm to exist.
    // There is NO #main-menu element — the site uses <ul class="sm"> elements.
    const pollForInstance = setInterval(() => {
        const $ = window.jQuery;
        if (!$) return;

        // Find all SmartMenus-initialized ULs
        const $menus = $('ul.sm');
        if (!$menus.length) return;

        // Check if at least one has been initialized by SmartMenus
        ...
        if (!anyInitialized) return;

        // === ALL INSTANCES FOUND — apply overrides NOW ===
        clearInterval(pollForInstance);

        // Instant show/hide functions (zero animation)
        const instantShow = function ($ul, complete) {
            $ul.css({ display: 'block', opacity: 1 });
            if (complete) complete();
        };
        const instantHide = function ($ul, complete) {
            $ul.css({ display: 'none' });
            if (complete) complete();
        };

        // Apply to EVERY SmartMenus instance on the page
        $menus.each(function () {
            const smData = $(this).data('smartmenus');
            if (smData) {
                smData.opts.showTimeout = 0;
                smData.opts.hideTimeout = 0;
                smData.opts.showFunction = instantShow;
                smData.opts.hideFunction = instantHide;
                smData.opts.collapsibleShowDuration = 0;
                smData.opts.collapsibleHideDuration = 0;
            }
        });

        // Also update global defaults for safety
        if ($.SmartMenus && $.SmartMenus.defaults) {
            $.SmartMenus.defaults.showTimeout = 0;
            $.SmartMenus.defaults.hideTimeout = 0;
            $.SmartMenus.defaults.showFunction = instantShow;
            $.SmartMenus.defaults.hideFunction = instantHide;
        }

        // Refresh all menus so deeply nested items re-read the new opts
        $menus.each(function () {
            if ($(this).data('smartmenus')) {
        ...
        // Prevent Bootstrap's click handler from fighting SmartMenus
        $('.navbar-inverse .dropdown-toggle').removeAttr('data-toggle');

        // Block clicks from toggling an already-visible hovering menu
        $('.navbar-inverse .has-submenu').each(function () {
            this.addEventListener('click', function (e) {
                const $sub = $(this).parent().children('ul');
        ...
        console.log('[FCW Navbar] SmartMenus patched on ' + $menus.length + ' ul.sm instances — all delays 0ms.');
    }, 30);

    // Safety: stop polling after 10 seconds
    setTimeout(() => clearInterval(pollForInstance), 10000);
})();
...
`;
        ...
        let baseBeta = null;  // Baseline when modal opens
        let baseGamma = null;

        // Smooth interpolation values
        let smoothRotateX = 0;
        let smoothRotateY = 0;
        const GYRO_LERP = 0.12; // Smoothing factor (0 = no movement, 1 = instant)
        const GYRO_MAX_ROTATION = 15; // Slightly more range than mouse for immersive feel
        const GYRO_SENSITIVITY = 0.6; // Scale factor for tilt-to-rotation

        function handleDeviceOrientation(event) {
            if (!modal.classList.contains('active') || isPaused) return;
            ...
            gyroRafId = requestAnimationFrame(() => {
                // Calculate delta from baseline (so the card starts centered)
                var deltaBeta = (currentBeta - baseBeta) * GYRO_SENSITIVITY;
                var deltaGamma = (currentGamma - baseGamma) * GYRO_SENSITIVITY;

                // Clamp to max rotation
                var targetRotateX = Math.max(-GYRO_MAX_ROTATION, Math.min(GYRO_MAX_ROTATION, -deltaBeta));
                var targetRotateY = Math.max(-GYRO_MAX_ROTATION, Math.min(GYRO_MAX_ROTATION, deltaGamma));
                ...
                // Update sparkle field
                var sparkleField = wrapper.querySelector('.fcw-scintillation-field');
                if (sparkleField) {
                    var danceFactorX = (smoothRotateY / GYRO_MAX_ROTATION) * -20;
                    var danceFactorY = (smoothRotateX / GYRO_MAX_ROTATION) * -20;
                    sparkleField.style.transform = 'translateZ(85px) translateX(' + danceFactorX.toFixed(1) + 'px) translateY(' + danceFactorY.toFixed(1) + 'px)';
                }

                // Update beam/streak effect
                var card = wrapper.querySelector('.fcw-cloned-card');
                if (card) {
                    var beamX = 50 + (smoothRotateY / GYRO_MAX_ROTATION) * 50;
                    var beamY = 50 + (smoothRotateX / GYRO_MAX_ROTATION) * 50;
                    var angle = Math.atan2(smoothRotateX, smoothRotateY) * (180 / Math.PI) + 180;
                ...
                DeviceOrientationEvent.requestPermission()
                    .then(function (state) {
                        if (state === 'granted') {
                            ...
                            startGyro();
                            console.log('[FCW Mobile] Gyroscope permission granted!');
                        } else {
                            console.log('[FCW Mobile] Gyroscope permission denied, using touch fallback.');
                        }
                    })
                    .catch(function (err) {
                        console.warn('[FCW Mobile] Gyro permission error:', err);
                    });
        ...
        function startGyro() {
            ...
            console.log('[FCW Mobile] Gyroscope tracking active.');
        }
        ...
        function stopGyro() {
            ...
            container.style.transform = 'rotateX(0deg) rotateY(0deg)';
            console.log('[FCW Mobile] Gyroscope tracking stopped.');
        }

        // --- TOUCH FALLBACK (drag to tilt) ---
        ...
        if (isMobileDevice) {
            stage.addEventListener('touchstart', function (e) {
                ...
                // Request gyro on first touch (iOS requires user gesture)
            ...
            stage.addEventListener('touchmove', function (e) {
                if (!modal.classList.contains('active') || isPaused || gyroActive) return;

                // Only use touch fallback if gyro is not active
                if (e.touches.length === 1) {
                    var rect = stage.getBoundingClientRect();
                    var dx = e.touches[0].clientX - touchStartX;
                    var dy = e.touches[0].clientY - touchStartY;

                    touchRotateY = Math.max(-12, Math.min(12, (dx / rect.width) * 24));
                    touchRotateX = Math.max(-12, Math.min(12, (dy / rect.height) * -24));
                    ...
                    var card = wrapper.querySelector('.fcw-cloned-card');
                    if (card) {
                        var beamX = 50 + (touchRotateY / 12) * 50;
                        var beamY = 50 + (touchRotateX / 12) * 50;
                        card.style.setProperty('--beam-x', beamX + '%');
                        card.style.setProperty('--beam-y', beamY + '%');
                    }
                }
            ...
            // Show a gyro hint toast on mobile when modal opens
            const origOpenModal = modal.classList.contains.bind(modal.classList);
            const gyroObserver = new MutationObserver(function (mutations) {
                mutations.forEach(function (m) {
                    if (m.attributeName === 'class') {
                        if (modal.classList.contains('active')) {
                            // Modal opened — start gyro if permitted
...
// === GYROSCOPE INSPECTION MODULE ===
// === FCWatch Overhaul — Mobile Gyroscope Inspection Module ===
// This module adds gyroscope-based card tilt to the inspection modal on iOS.
// It watches for the inspector modal to open, then maps phone orientation to card rotation.

(function __fcwGyroInspect() {
    'use strict';
    ...
    console.log('[FCW Gyro] Mobile device detected, enabling gyro inspection module.');

    // --- Config ---
    var GYRO_MAX_ROTATION = 35; // degrees
    var GYRO_LERP = 0.15;      // smoothing factor
    var GYRO_MULTIPLIER = 2.5; // how sensitive the tilt is

    // --- State ---
    var gyroActive = false;
    var gyroPermission = false;
    var baseBeta = null;
    var baseGamma = null;
    var currentRotX = 0;
    var currentRotY = 0;
    ...
    var inspectCard = null;

    // --- Request iOS 13+ Gyro Permission on First Interaction ---
    function requestGyroPermission() {
        ...
        if (typeof DeviceOrientationEvent !== 'undefined' &&
            ...
            DeviceOrientationEvent.requestPermission()
                .then(function (response) {
                    if (response === 'granted') {
                        gyroPermission = true;
                        if (inspectCard) startGyro();
                        console.log('[FCW Gyro] Permission granted!');
                    } else {
                        console.log('[FCW Gyro] Permission denied, using touch fallback.');
                    }
                })
                .catch(function (err) {
                    // Silently fail if not a user gesture yet
                });
        } else {
            // Non-iOS or older iOS — gyro is available without permission
            gyroPermission = true;
        }
    }
    ...
    document.addEventListener('touchend', requestGyroPermission, { once: true, capture: true });

    // --- Gyro Handler ---
    function handleOrientation(event) {
        if (!gyroActive || !inspectCard) return;
        ...
        // Calculate delta from baseline and apply sensitivity multiplier
        var deltaBeta = (beta - baseBeta) * GYRO_MULTIPLIER;
        var deltaGamma = (gamma - baseGamma) * GYRO_MULTIPLIER;

        // Clamp to max rotation
        targetRotX = Math.max(-GYRO_MAX_ROTATION, Math.min(GYRO_MAX_ROTATION, -deltaBeta));
        targetRotY = Math.max(-GYRO_MAX_ROTATION, Math.min(GYRO_MAX_ROTATION, deltaGamma));
    }

    // --- Smooth Animation Loop ---
    function animateGyro() {
        if (!gyroActive || !inspectCard) {
            animFrameId = null;
            return;
        }

        // Lerp toward target
        currentRotX += (targetRotX - currentRotX) * GYRO_LERP;
        currentRotY += (targetRotY - currentRotY) * GYRO_LERP;

        // Apply transform to the card
        inspectCard.style.transform =
            'perspective(1000px) rotateX(' + currentRotX.toFixed(2) + 'deg) rotateY(' + currentRotY.toFixed(2) + 'deg)';

        animFrameId = requestAnimationFrame(animateGyro);
    }
    ...
    function startGyro() {
        ...
        animFrameId = requestAnimationFrame(animateGyro);
        console.log('[FCW Gyro] Gyroscope tracking started.');
    }
    ...
    function stopGyro() {
        ...
        inspectCard = null;
        console.log('[FCW Gyro] Gyroscope tracking stopped.');
    }
    ...
    function onTouchMove(e) {
        if (!inspectCard || !isTouching || gyroActive) return;
        var touch = e.touches[0];
        var dx = touch.clientX - touchStartX;
        var dy = touch.clientY - touchStartY;

        var rotY = Math.max(-GYRO_MAX_ROTATION, Math.min(GYRO_MAX_ROTATION, dx * 0.3));
        var rotX = Math.max(-GYRO_MAX_ROTATION, Math.min(GYRO_MAX_ROTATION, -dy * 0.3));
    ...
    function onTouchEnd() {
        ...
        if (!gyroActive) {
            // Spring back
            inspectCard.style.transition = 'transform 0.4s ease-out';
            inspectCard.style.transform = 'perspective(1000px) rotateX(0deg) rotateY(0deg)';
            setTimeout(function () {
                if (inspectCard) inspectCard.style.transition = '';
            }, 400);
        }
    }

    // --- Watch for the Inspector Modal to Open/Close ---
    function findInspectCard() {
        // Look for common inspector modal selectors used by FCWatch
        var candidates = [
            '.inspect-modal .card-image',
            '.inspect-modal img',
            '.modal-card-image',
            '#inspectModal .card',
            '#inspectModal img',
            '.inspectContainer img',
            '.inspectContainer',
            '.inspect-card',
            '[class*="inspect"] img',
            '.modal-content img'
        ...
        // Fallback: look for any large centered image in a modal/overlay
        var modals = document.querySelectorAll('[class*="modal"], [class*="overlay"], [class*="inspect"]');
        for (var j = 0; j < modals.length; j++) {
            var imgs = modals[j].querySelectorAll('img');
            for (var k = 0; k < imgs.length; k++) {
                if (imgs[k].offsetWidth > 100) return imgs[k];
            }
        }
    ...
    var observer = new MutationObserver(function (mutations) {
        for (var i = 0; i < mutations.length; i++) {
            ...
            // Check for added nodes (modal opening)
            if (mutation.addedNodes.length > 0) {
                setTimeout(function () {
                    var card = findInspectCard();
                    if (card && !inspectCard) {
                        inspectCard = card;
                        console.log('[FCW Gyro] Inspect modal detected, activating gyro.');

                        // Add touch listeners to the card
                        inspectCard.addEventListener('touchstart', onTouchStart, { passive: true });
                        inspectCard.addEventListener('touchmove', onTouchMove, { passive: true });
                        inspectCard.addEventListener('touchend', onTouchEnd, { passive: true });

                        // Show hint
                        var hint = document.createElement('div');
                        hint.style.cssText = 'position:fixed;bottom:80px;left:50%;transform:translateX(-50%);background:rgba(0,0,0,0.8);color:#00ffcc;padding:10px 20px;border-radius:20px;font-size:13px;z-index:999999;opacity:0;transition:opacity 0.3s;font-family:-apple-system,sans-serif;';
                        hint.textContent = '📱 Tilt your phone to inspect the card';
                        ...
                        // Auto-request gyro on modal open
                        if (!gyroPermission) {
                            // Can't auto-request, need user gesture — touch will trigger it
                        } else {
                            startGyro();
                        }
                    }
            ...
            // Check for removed nodes (modal closing)
            if (mutation.removedNodes.length > 0) {
                if (inspectCard && !document.body.contains(inspectCard)) {
                    stopGyro();
                }
            }
        }
    ...
    console.log('[FCW Gyro] Gyroscope inspection module loaded — watching for inspect modal.');
})();
...
// Specifically targets Packs, Leaderboards, and general layout.

(function __fcwMobileCSS() {
    'use strict';

    // Only run on mobile devices
    var isMobile = ('ontouchstart' in window) || (navigator.maxTouchPoints > 0);
    // Even if not mobile touch, if screen is small we should apply it
    ...
    style.textContent = `
        ...
        @media (max-width: 768px) {
            /* Fix horizontal scrolling and padding */
            html, body { 
                overflow-x: hidden !important; 
            ...
            /* Force Bootstrap columns to wrap appropriately on mobile */
            .row { 
            ...
            /* Default to 2-columns (50%) for medium/large grids so it doesn't look empty, except for full-width elements */
            [class*="col-"] { 
                padding-left: 5px !important; 
                padding-right: 5px !important; 
                margin-bottom: 15px !important; 
            }
            .col-md-2, .col-md-3, .col-sm-3, .col-lg-2, .col-lg-3 {
                width: 50% !important; 
            ...
            /* Keep explicitly requested small utility grids intact */
            .col-xs-6, .col-sm-6, .col-6 { flex: 0 0 50% !important; max-width: 50% !important; }
            .col-xs-4, .col-sm-4, .col-4 { flex: 0 0 33.333% !important; max-width: 33.333% !important; }
            .col-xs-3, .col-xs-2 { flex: 0 0 25% !important; max-width: 25% !important; }
            ...
            /* Make images responsive but ONLY inside content containers, so we don't blow up the navbar logo */
            .container img, .pack-container img, .modal-dialog img, td img { 
                max-width: 100% !important; 
                /* Remove height: auto parameter here so we don't accidentally override inline width/height attributes like height="40" */
            }
            ...
            /* specific override for pack openings which might be wide */
            .pack-opening-area {
                display: flex;
                flex-direction: column;
                align-items: center;
            ...
            .hit-label {
                ...
                bottom: 60px !important; /* Move above the hit zone */
            }
            .lny-feedback {
    ...
    // Prevent duplicate style tags on SPA layout re-injects
    if (document.getElementById('fcw-mobile-responsive-css')) {
        document.getElementById('fcw-mobile-responsive-css').remove();
    }

    document.head.appendChild(style);
    console.log('[FCW Mobile] Global responsiveness CSS injected for Leaderboards & Packs.');
})();


// === PERSISTENCE MODULE ===
// === FCWatch Overhaul — Persistence Module ===
// Intercepts all navigation within fc-watch.com so the bookmarklet stays active.
// Instead of doing full page reloads (which kill the bookmarklet), this module
// fetches new pages via AJAX, swaps the body, and re-initializes FCWatch.

(function __fcwPersist() {
    'use strict';

    var SCRIPT_SRC = 'https://slimerewinston.github.io/fcwatch-overhaul-mobile.js';

    // Set localStorage flag
    try { localStorage.setItem('__fcw_persist', 'true'); } catch (e) { }

    function reinjectFCW() {
        // We do NOT want the entire bookmarklet to run global setups again.
        // We only want the page-specific DOM manipulative functions to run.
        // The top of the bundle uses `__FCW_BOOKMARKLET_LOADED` as a guard.
        // If we bypass it, we get exponential execution. 
        // Best approach: Just dispatch a custom event that the scripts can listen to for re-initialization!
        document.dispatchEvent(new CustomEvent('fcw-spa-navigate'));

        // We still need to re-run the main script if it was designed to run once on load.
        // Let's rely on the bookmarklet's internal functions instead of re-injecting the massive file.
        // Actually, re-injecting the script but with a flag is safest.
        window.__FCW_SPA_REINJECT = true;
        window.__FCW_BOOKMARKLET_LOADED = false;
        var s = document.createElement('script');
        s.src = SCRIPT_SRC + '?t=' + Date.now();
        document.head.appendChild(s);
    }

    if (window.__FCW_PERSIST_INIT_DONE) return;
    window.__FCW_PERSIST_INIT_DONE = true;

    // Intercept all link clicks
    document.addEventListener('click', function (e) {
        // If the site's native JS already intercepted this click (e.g. to open a modal), do not interfere!
        ...
        try {
            ...
            if (!url.hostname.includes('fc-watch.com')) return;
            // Ignore anchor links on the same page (often used for tabs/modals)
            if (url.pathname === window.location.pathname && url.hash) return;
        } catch (err) { return; }

        if (href.startsWith('javascript:')) return;

        // Whitelist specific classes used for modals or cards so we don't destroy them
        var classStr = ((link.className || '') + ' ' + (link.parentElement ? link.parentElement.className : '')).toLowerCase();
        if (classStr.includes('card') || classStr.includes('inspect') || classStr.includes('modal') || classStr.includes('player')) {
            console.log('[FCW Persist] Ignored click on card/modal element');
            return;
        }
        ...
        loader.style.cssText = 'position:fixed;top:0;left:0;width:100%;height:3px;background:linear-gradient(90deg,#00ccff,#0066ff);z-index:999999;animation:__fcwL 1s infinite;';
        var st = document.createElement('style');
        st.textContent = '@keyframes __fcwL{0%{transform:scaleX(0);transform-origin:left}50%{transform:scaleX(1);transform-origin:left}50.01%{transform-origin:right}100%{transform:scaleX(0);transform-origin:right}}';
        ...
        fetch(href, { credentials: 'include' })
            ...
            .then(function (html) {
                ...
                window.history.pushState({ __fcw: true }, document.title, href);
                window.scrollTo(0, 0);
                setTimeout(function () { reinjectFCW(); }, 300);
            })
            .catch(function (err) {
                console.error('[FCW Persist] Fetch failed:', err);
                if (loader.parentNode) loader.remove();
                window.location.href = href;
            });
    }, false); // <--- Use bubbling phase (false) instead of capture (true) so the site's own handlers can run first and call preventDefault()
    ...
    console.log('[FCW Mobile] Persistence active — bookmarklet stays on across pages!');
})();


    // === SUCCESS NOTIFICATION ===
    (function showToast() {
        if (window.__FCW_SPA_REINJECT) return; // Do not show on background page navigations
        if (document.getElementById('fcw-mobile-toast')) return;
